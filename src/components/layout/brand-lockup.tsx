...
export function BrandLockup({ compact = false, className }: BrandLockupProps) {
  return (
    <div className={cn("flex items-center gap-4", className)}>
      <div className={cn("relative overflow-hidden rounded-[1.35rem] border border-border/70 bg-card shadow-sm", compact ? "size-11" : "size-14")}>
        <Image src="/logomark.png" alt="Gliss logo" fill className="object-contain p-1" priority />
      </div>
      {!compact ? (
        <div className="relative h-12 w-[190px] sm:w-[220px]">
